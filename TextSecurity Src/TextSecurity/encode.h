...
static char *wsgets(char *buf, int size, FILE *fp)
{
	int		n;
...
static BOOL wsputs (char *buf,FILE *fp)
{
	int		len = strlen (buf);
...
static void whitespace_storage(const char	*buf, unsigned long	*n_lo,unsigned long	*n_hi)
{
...
static void encode_buffer_load (FILE *fp)
{
	int	 i;
...
static BOOL encode_append_whitespace(int nsp)
{
...
static BOOL
encode_write_value ( int	 val, FILE *inf,FILE *outf)
{
	int		nspc;
...
static BOOL
encode_write_flush (FILE *inf,FILE *outf)
{
	char		buf[BUFSIZ];
...
BOOL
encode_bit (int	 bit,FILE *inf,FILE *outf)
{
...
BOOL encode_flush (FILE *inf,FILE *outf)
{
	...
	if (!quiet_flag)
	{
		...
		{
			char te[100];
		   sprintf (te, "Message exceeded available space by approximately %.2f%%.\n",
	         ((double) encode_bits_used / encode_bits_available - 1.0) * 100.0);
		   strcat(ErroeM,te);
		  sprintf (te, "An extra %ld lines were added.\n", encode_lines_extra);
		   strcat(ErroeM,te);
	    } 
		else
		{
...
static BOOL decode_bits (int	 spc,FILE *outf)
{
	int		b1 = 0, b2 = 0, b3 = 0;
	...
	{
		 char te[100];
	        sprintf (te, "Illegal encoding of %d spaces\n", spc);
			strcat(ErroeM,te);
...
static BOOL decode_whitespace (const char	*s,FILE *outf)
{
	int		spc = 0;
...
BOOL
message_extract (FILE *inf,FILE *outf)
{
	char		buf[BUFSIZ];
...
void space_calculate (FILE *fp)
{
	...
	{
		char te[100];
	    sprintf (te,"File has storage capacity of %ld bits (%ld bytes)\n", n_lo, n_lo / 8);
		strcat(ErroeM,te);
	} 
	else 
	{
		 char te[100];
		  sprintf (te,"File has storage capacity of between %ld and %ld bits.\n", n_lo, n_hi);
		  strcat(ErroeM,te);
	     sprintf (te,"Approximately %ld bytes.\n", (n_lo + n_hi) / 16);
