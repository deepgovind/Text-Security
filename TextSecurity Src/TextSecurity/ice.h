...
ICE_KEY *ice_key_create (int n)
{
	...
	if ((ik->ik_keysched = (ICE_SUBKEY *) malloc (ik->ik_rounds * sizeof (ICE_SUBKEY))) == NULL)
	{
	    free (ik);
	    return (NULL);
	}
...
static unsigned long ice_f (register unsigned long	p,const ICE_SUBKEY	sk)
{
...
void ice_key_encrypt (const ICE_KEY *ik,const unsigned char	*ptext,unsigned char *ctext)
{
	register int		i;
...
void ice_key_decrypt (const ICE_KEY *ik,const unsigned char	*ctext,unsigned char *ptext)
{
	register int		i;
	register unsigned long	l, r;
...
static void ice_key_sched_build (ICE_KEY *ik,unsigned short	*kb,int n,const int	*keyrot)
{
	int		i;
